...
export function buildGameCanvas() {
  ...
  var _animations = {
    red: { frames: [0], speed: 0.3 },
    green: { frames: [1], speed: 0.3 },
  };
  ...
  for (var n = 0; n <= gameData.totalRound; n++) {
    ...
    for (var key in gameSettings["game" + n].items) {
      ...
      if (key == "doll") {
        ...
        var _animations = {
          idle: { frames: [0, 1], speed: 0.3 },
          close: { frames: [2], speed: 0.3 },
          peek: { frames: [3], speed: 0.3 },
        };
  ...
  for (var n = 0; n < players_arr.length; n++) {
    ...
    var _animations = {
      idle: { frames: [0, 1], speed: 0.3 },
      run: { frames: [2, 3], speed: 0.3 },
      dead: { frames: [4, 5, 6], speed: 0.3 },
      pull: { frames: [7, 8], speed: 0.3 },
      front: { frames: [9, 10], speed: 0.3 },
      frontrun: { frames: [11, 12], speed: 0.3 },
      frontpull: { frames: [13, 14], speed: 0.3 },
    };
  ...
  for (var n = 0; n < 2; n++) {
    ...
    $.sprites["handStat" + n].addChild(
      $.sprites["handMarbleBall" + n],
      $.sprites["handMarbleBallTxt" + n],
      $.sprites["handMarbleBallPlayerTxt" + n]
    );
  ...
  for (var n = 0; n < 3; n++) {
    ...
    $.sprites["marbleBall" + n].scaleX = $.sprites["marbleBall" + n].scaleY = 0.7;
  ...
  for (var n = 0; n < 2; n++) {
    ...
    $.sprites["healthBg" + n].graphics
      .beginFill(gameSettings.game6.bar.background)
      .drawRect(
        -defaultGameData.survivalBarBorder,
        -defaultGameData.survivalBarBorder,
        defaultGameData.survivalBarW + defaultGameData.survivalBarBorder * 2,
        defaultGameData.survivalBarH + defaultGameData.survivalBarBorder * 2
      );

    $.sprites["healthEmpty" + n] = new createjs.Shape();
    $.sprites["healthEmpty" + n].graphics
      .beginFill(gameSettings.game6.bar.empty)
      .drawRect(0, 0, defaultGameData.survivalBarW, defaultGameData.survivalBarH);

    $.sprites["healthGreen" + n] = new createjs.Shape();
    $.sprites["healthGreen" + n].graphics
      .beginFill(gameSettings.game6.bar.health)
      .drawRect(0, 0, defaultGameData.survivalBarW, defaultGameData.survivalBarH);

    $.sprites["healthBlood" + n] = new createjs.Shape();
    $.sprites["healthBlood" + n].graphics
    ...
    $.sprites["healthBar" + n].addChild(
      $.sprites["healthBg" + n],
      $.sprites["healthEmpty" + n],
      $.sprites["healthBlood" + n],
      $.sprites["healthGreen" + n]
    );
  ...
  optionsContainer.addChild(
    ...
    buttonExit,
    buttonSettings
  ); // Add buttonSettings to optionsContainer
  ...
  piggyContainer.scaleX = piggyContainer.scaleY = 0.8;
...
export function changeCanvasViewport(contentW, contentH) {
  if (canvasContainer != undefined) {
    ...
    if (viewport.isLandscape) {
      ...
      handHandsContainer.scaleX = handHandsContainer.scaleY = 0.7;
...
function createHitarea(obj) {
  obj.hitArea = new createjs.Shape(
    new createjs.Graphics().beginFill("#000").drawRect(0, 0, obj.image.naturalWidth, obj.image.naturalHeight)
  );
}
