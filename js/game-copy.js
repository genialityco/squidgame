...
export var gameSettings = {
  ...
  game2: {
    ...
    candy: [
      {
        src: "assets/candy_01.png",
        finalSrc: "assets/candy_final_01.png",
        checkpoint: [
          { x: 95, y: 340 },
          { x: 397, y: 340 },
          { x: 246, y: 85 },
      ...
      {
        src: "assets/candy_02.png",
        finalSrc: "assets/candy_final_02.png",
        checkpoint: [
          { x: 244, y: 90 },
          { x: 407, y: 209 },
          { x: 345, y: 396 },
      ...
      {
        src: "assets/candy_03.png",
        finalSrc: "assets/candy_final_03.png",
        checkpoint: [
          { x: 243, y: 86 },
          { x: 125, y: 348 },
          { x: 369, y: 334 },
      ...
      {
        src: "assets/candy_04.png",
        finalSrc: "assets/candy_final_04.png",
        checkpoint: [
          { x: 243, y: 64 },
          { x: 428, y: 228 },
          { x: 208, y: 416 },
...
export function moveFrontPlayer() {
  ...
  var getPlayerIndex = roundData.bridgeData.turnArr.indexOf(
  ...
  var posData = {
    z: player.z + defaultGameData.bridgeSteps * defaultData.segmentLength,
    offset: newOffset,
  };
  ...
  var newCameraPostion =
    defaultData.position +
    defaultGameData.bridgeSteps * defaultData.segmentLength;
...
function animatePlayerJump(player, posData, con) {
  ...
  var totalLength =
    gameSettings.game5.length *
    defaultGameData.bridgeSteps *
    defaultData.segmentLength;
  totalLength += (defaultGameData.bridgeStart - 2) * defaultData.segmentLength;
  var moveSpeed =
    player.z > (defaultGameData.bridgeStart - 2) * defaultData.segmentLength
      ? 1
      : 0.5;
  ...
  if (moveSpeed == 1) {
    ...
    TweenMax.to(player.bounceData, moveSpeed / 4, {
      y: 300,
      ease: Power2.easeOut,
    });
    TweenMax.to(player.bounceData, moveSpeed / 2, {
      y: 0,
      ease: Bounce.easeOut,
      delay: moveSpeed / 4,
    });
  ...
  TweenMax.to(player.tweenData, moveSpeed, {
    ...
    onComplete: function () {
      ...
      if (con) {
        ...
        if (
          roundData.bridgeData.seqArrIndex < roundData.bridgeData.seqArr.length
        ) {
          if (
            roundData.bridgeData.seqArr[roundData.bridgeData.seqArrIndex]
              .side != checkSide
          ) {
            roundData.bridgeData.newPos.offset =
              posData.offset > 0 ? -0.2 : 0.2;
        ...
        if (roundData.bridgeData.fail) {
          if (
            roundData.bridgeData.playerIndex >=
            roundData.bridgeData.turnArr.length - 1
          ) {
            endGame(false, false);
          }
        }
...
function moveOtherPlayers(con) {
  ...
  var animateCount = 0;
  for (
    var n = roundData.bridgeData.playerIndex;
    n < roundData.bridgeData.turnArr.length;
    n++
  ) {
...
export function initCandyDrawingPos(x, y) {
  ...
  for (
    var n = 0;
    n < gameSettings["game2"].candy[candyNum].checkpoint.length;
    n++
  ) {
    roundData.candyData.checkpoint.push({
      x:
        gameSettings["game2"].candy[candyNum].checkpoint[n].x -
        itemCandyBase.image.naturalWidth / 2,
      y:
        gameSettings["game2"].candy[candyNum].checkpoint[n].y -
        itemCandyBase.image.naturalHeight / 2,
      done: false,
    });
  }
}
...
export function checkCandyDrawingPos(x, y) {
  ...
  for (var n = 0; n < roundData.candyData.checkpoint.length; n++) {
    var getTwoDistance = getDistance(
      x,
      y,
      roundData.candyData.checkpoint[n].x,
      roundData.candyData.checkpoint[n].y
    );
  ...
  if (doneCount >= roundData.candyData.checkpoint.length) {
    var getTwoDistance = getDistance(
      x,
      y,
      roundData.candyData.x,
      roundData.candyData.y
    );
    if (getTwoDistance < distanceNum) {
...
export function chooseRandomCandy() {
  ...
  roundData.candyData.candyNum = Math.floor(
    Math.random() * gameSettings.game2.candy.length
  );
...
export function revealCandy() {
  ...
  TweenMax.to($.sprites["candy" + roundData.candyData.candyNum], 0.5, {
    alpha: 0,
    overwrite: true,
  });
...
export function changeMarbleTurn() {
  ...
  TweenMax.to($.sprites["handWrap" + roundData.marbleData.guessNum], 0.5, {
    bezier: { curviness: 1, values: pathArr },
...
function showMarbleResult(result) {
  ...
  if (roundData.marbleData.turn == 1) {
    tweenSpeed = 1.3;
    $.sprites["handWinStatTxt" + roundData.marbleData.guessNum].text =
      resultTxt;
  }

  TweenMax.to(handContainer, tweenSpeed, {
    overwrite: true,
    onComplete: function () {
      $.sprites["handWinStatTxt0"].text = "";
      $.sprites["handWinStatTxt1"].text = "";
      ...
      for (var n = 0; n < totalBall; n++) {
        ...
        $.sprites["marbleBall" + n].x =
          guessX + randomInt(-randomDis, randomDis) + n * 20;
        $.sprites["marbleBall" + n].y =
          guessY + randomInt(-randomDis, randomDis) + n * 20;
      }

      TweenMax.to(handContainer, 1, {
        overwrite: true,
        onComplete: function () {
          $.sprites["hand" + roundData.marbleData.chooseNum].sprite.gotoAndStop(
            2
          );
          ...
          if (roundData.marbleData.result == result) {
            ...
            TweenMax.to($.sprites["marbleBall" + randomBall], 0.5, {
              x: chooseX,
              overwrite: true,
              onComplete: function () {
                endMarbleTurn(true);
            ...
            TweenMax.to($.sprites["marbleBall" + 2], 0.5, {
              x: guessX,
              overwrite: true,
              onComplete: function () {
                endMarbleTurn(false);
...
export function swithTurn() {
  ...
  TweenMax.to($.sprites["healthType" + 0], 0.5, {
    x: $.sprites["healthType" + 0].oriX,
    alpha: 1,
    overwrite: true,
  });
  ...
  TweenMax.to($.sprites["healthType" + 1], 0.5, {
    x: $.sprites["healthType" + 1].oriX,
    alpha: 1,
    overwrite: true,
  });
...
function getOppAway() {
  ...
  if (Math.abs(distanceX) < rangeNumX - 0.1) {
    if (distanceX < 0) {
      roundData.survivalData.oppData.offset =
        targetPlayer.offset + roundData.survivalData.oppData.speedX;
...
export function updatePlayerHealthBar() {
  var userBar =
    (roundData.survivalData.userHealth / 100) * defaultGameData.survivalBarW;
  var oppBar =
    (roundData.survivalData.oppHealth / 100) * defaultGameData.survivalBarW;
  ...
  for (n = 0; n < players.length; n++) {
    ...
    if (gameData.roundNum == 1) {
      ...
      var endZ =
        (defaultGameData.tugStart +
          defaultGameData.tugHole +
          defaultGameData.tugGap / 2) *
        worldData.segmentLength;
      ...
      if (n == 0) {
        ...
        if (player.status == "") {
          if (roundData.survivalData.move) {
            ...
            if (
              stage.mouseY >
              (canvasH / 100) * defaultGameData.survivalSplitPercent
            ) {
      ...
      if (
        player.z <
        defaultGameData.survivalStart * defaultData.segmentLength
      ) {
...
function updatePlayerOffset(player, oldSegment, playerSegment, playerW) {
  var i,
    ...
    otherPlayerW,
    lookahead = 20,
    playerW = player.sprite.w * defaultData.scale;
  ...
  for (i = 1; i < lookahead; i++) {
    ...
    if (
      ...
      player.speed > defaultData.speed &&
      getOverlap(defaultData.playerX, playerW, player.offset, playerW, 1.2)
    ) {
      if (defaultData.playerX > 0.5) dir = -1;
      else if (defaultData.playerX < -0.5) dir = 1;
      else dir = player.offset > defaultData.playerX ? 1 : -1;
      return (
        (((dir * 1) / i) * (player.speed - defaultData.speed)) /
        worldData.maxSpeed
      );
    }
    ...
    for (j = 0; j < segment.players.length; j++) {
      ...
      if (
        player.speed > otherPlayer.speed &&
        getOverlap(
          player.offset,
          playerW,
          otherPlayer.offset,
        ...
        if (otherPlayer.offset > 0.5) dir = -1;
        else if (otherPlayer.offset < -0.5) dir = 1;
        else dir = player.offset > otherPlayer.offset ? 1 : -1;
        return (
          (((dir * 1) / i) * (player.speed - otherPlayer.speed)) /
          worldData.maxSpeed
        );
      }
    }
  ...
  if (player.offset < -0.9) return 0.1;
  else if (player.offset > 0.9) return -0.1;
  else return 0;
}
